...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc+imm; if(rd!=0)trace_func_call(s->pc, s->dnpc));
  
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = src1 + ((imm>>1)<<1); R(rd) = s->pc+4; if(s->isa.inst.val == 0x00008067)trace_func_ret(s->pc, s->dnpc); else if(rd == 1)trace_func_call(s->pc, s->dnpc); else if(rd == 0 && imm == 0)trace_func_call(s->pc, s->dnpc););
