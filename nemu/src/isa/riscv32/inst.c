...
*imm = SEXT(((___x<<10) | BITS(i,30,21)) << 1, 20);} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + SEXT(imm,12), 1));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + SEXT(imm,12), 1, src2));
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc+(SEXT(imm,20)<<1));
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, s->dnpc = src1 + ((SEXT(imm,12)>>1)<<1); R(rd) = s->pc+4);
  INSTPAT("??????? ????? ????? ??? ????? 00000 11", lw     , I, R(rd) = Mr(src1 + SEXT(imm,12), 4));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", BEQ    , B, s->dnpc=((src1 == src2) ? (s->pc + SEXT(imm<<1,13)) : s->dnpc));
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", BGE    , B, s->dnpc=((src1 >= src2) ? (s->pc + SEXT(imm<<1,13)) : s->dnpc));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", BNE    , B, s->dnpc=((src1 != src2) ? (s->pc + SEXT(imm<<1,13)) : s->dnpc));
