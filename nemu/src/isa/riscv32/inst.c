...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", BEQ    , B, s->dnpc=((src1 == src2) ? (s->pc + SEXT(imm,12)) : s->dnpc));
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", BGE    , B, s->dnpc=((src1 >= src2) ? (s->pc + SEXT(imm,12)) : s->dnpc));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", BNE    , B, s->dnpc=((src1 != src2) ? (s->pc + SEXT(imm,12)) : s->dnpc));
