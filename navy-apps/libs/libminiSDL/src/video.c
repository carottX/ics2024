...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect) {
  ...
  if(srcrect != NULL) {
    start_x = srcrect->x, start_y = srcrect -> y;
    src_w = srcrect->w, src_h = srcrect -> h;
  }
  for(int j=0; j<src_w; ++j) {
    for(int i=0; i<src_h; ++i) {
      dst->pixels[(dest_y+i) * dst->w + dest_x + j] = src->pixels[(start_y + i) * src->w + start_x + j];
    }
  }
}
...
void SDL_UpdateRect(SDL_Surface *s, int x, int y, int w, int h) {
  ...
  //       tmp[i*w + j] = color.a << 24 | color.r << 16 | color.g << 8 | color.b;
  //     }
  //   }
  ...
  //   for(int i=0; i<h; ++i){
  //     for(int j=0; j<w; ++j){
  //       tmp[i*w + j] =  s->pixels[4*((i+y) * s->w + j + x) + 3] << 24 | s->pixels[4*((i+y) * s->w + j + x) + 2] << 16 | s->pixels[4*((i+y) * s->w + j + x) + 1] << 8 | s->pixels[4*((i+y) * s->w + j + x)];
  //     }
  //   }
  //   NDL_DrawRect(tmp, x, y, w, h);
  //   free(tmp);
